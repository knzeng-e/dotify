...
The former `VITE_DOTIFY_HOST_LINEUP` local preview has been promoted into the normal room protocol. The host can order up to 12 catalog tracks; listeners receive the same ephemeral metadata-only lineup, late joiners receive its current snapshot, and the list dies with the room. Next and natural track endings consume the ordered list before returning to catalog order. Every opened track still passes through the existing access check; the lineup never carries source references, keys, payments, or access claims. A locked selection stops at the existing host-facing access gate and never initiates payment. Participant requests remain separate until the host deliberately adds the corresponding catalog track.
