...
unset/`viem`, so those actions continue to require an EVM wallet until
host-signed transaction evidence is captured. The older passkey-derived wallet
route is retired from public Product and standalone flows.
...
| Classic payment       | Native runtime payment intent through `RuntimeWritePort`; the asset label is derived from the connected EVM `chainId` (`PAS` on Product DevNet/Paseo, `DOT` on a DOT-backed Polkadot Hub EVM chain) | EVM wallet in the tracked build; Product CDM writer only when `VITE_DOTIFY_RUNTIME_ADAPTER=product-cdm` is explicitly enabled | CASH settlement after the Product payment rail is designed                             |
...
CASH-to-runtime entitlement path. The proof must derive the payer H160 from the
Product public key, identify the exact finalized CASH transfer, and correlate
the Asset Hub entitlement to that same receipt rather than accepting pre-existing
runtime access:
...
selection as reads: `viem` uses the connected EVM wallet, and
