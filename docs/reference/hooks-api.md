...
|---|---|---|
| `selectTrack` | `(track: CatalogTrack) => Promise<void>` | Select a track, resolve its audio URL, check access. Updates `audioSource`, `trackInfo`, `accessGate`. |
| `openTrack` | `(track: CatalogTrack) => Promise<void>` | Navigate to the player view, then call `selectTrack`. |
...
| Function | Signature | Description |
|---|---|---|
| `createSession` | `(event?: FormEvent<HTMLFormElement>) => void` | Create a new room as host. Emits `room:create`. |
