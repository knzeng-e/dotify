...
| ---------------------------- | -------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `selectTrack`                | `(track: CatalogTrack, socketEmit?, setLocalStreamReady?, closeHostPeers?) => Promise<RoomPlaybackMode>` | Select a track, resolve its audio URL, check access, and return `full` or `preview` for host room metadata. Updates `audioSource`, `trackInfo`, `accessGate`. |
...
| `setupPreviewLimit`          | `() => void`                                                                                             | Called on `loadedmetadata`. Sets the 42 % preview cutoff in `previewLimitRef`.                                                                                |
| `enforcePreviewCutoff`       | `() => void`                                                                                             | Called on `timeupdate` and `play`. Pauses audio and shows `AccessGate` when the limit is reached.                                                             |
...
| `roomPlaybackMode`  | `RoomPlaybackMode` | Host-declared room playback mode: `full` or `preview`         |
