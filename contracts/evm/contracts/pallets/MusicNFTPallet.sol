...
contract MusicNFTPallet {
  ...
  function musicNFTTransfer(uint256 tokenId, address to) external {
    ...
    if (track.accessMode == LibMusicRegistry.AccessMode.HumanFree) {
      require(LibMusicAccess.hasRequiredPersonhood(LibMusicAccess.store(), to, track.requiredPersonhood), 'MusicNFT: recipient lacks required personhood');
    }
  ...
  function musicNFTApprove(address to, uint256 tokenId) external {
    ...
    require(msg.sender == owner || ns.operatorApprovals[owner][msg.sender], 'MusicNFT: not owner or operator');
