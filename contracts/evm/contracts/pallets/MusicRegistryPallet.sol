...
contract MusicRegistryPallet {
    ...
    function musicRegRegister(
        TrackRegistration calldata reg,
        ...
        LibMusicRegistry.Storage storage rs2 = LibMusicRegistry.store();
        rs2.nextTokenId += 1;
        uint256 tokenId = rs2.nextTokenId;

        LibMusicNFT.mint(LibMusicNFT.store(), msg.sender, tokenId);
        rs2.tokenIdToHash[tokenId] = reg.contentHash;
        rs2.trackHashes.push(reg.contentHash);

        LibMusicRegistry.TrackRecord storage track = rs2.tracks[reg.contentHash];
        ...
        emit TrackRegistered(
            reg.contentHash, tokenId, msg.sender, reg.title,
            reg.accessMode, track.pricePlanck, track.requiredPersonhood
        );
    }
    ...
    function musicRegGetTrack(bytes32 contentHash)
        external view
        returns (LibMusicRegistry.TrackRecord memory track, address tokenOwner)
    {
        LibMusicRegistry.Storage storage rs = LibMusicRegistry.store();
        LibMusicRegistry.requireExists(rs, contentHash);
    ...
    function musicRegGetTrackByTokenId(uint256 tokenId)
        external view
        returns (LibMusicRegistry.TrackRecord memory track, address tokenOwner)
    {
        LibMusicRegistry.Storage storage rs = LibMusicRegistry.store();
    ...
    function _validateRegistration(
        LibMusicRegistry.Storage storage rs,
        TrackRegistration calldata reg
    ) private view {
        ...
        require(
            reg.accessMode == LibMusicRegistry.AccessMode.HumanFree || reg.pricePlanck > 0,
            "MusicRegistry: Classic requires price"
        );
        require(
